...
for (ii in 1:simN){
  ...
  for(j in 1:length(alphalist)){
    ...
    full_fit_no_pf = cv.multiview(list(x1=train_X1,x2=train_X2,x3=train_X3),train_y,
                                   rho=alpha,foldid=foldid)
    
    yhat_coop_new_no_pf = predict(full_fit_no_pf, list(x1=train_X1,x2=train_X2,x3=train_X3), s="lambda.min")
    new_train_coop_no_pf[ii,j] = calc_mse(yhat_coop_new_no_pf, mu_train)
    
    if (fit_mode == 'min'){
      cvm_min_ind = which.min(full_fit_no_pf$cvm)
      new_cv_coop_no_pf[ii, j] = min(full_fit_no_pf$cvm) #full_fit_no_pf$cvm[full_fit_no_pf$ind_min]
      cvm_min_no_pf[j] = min(full_fit_no_pf$cvm) #full_fit_no_pf$cvm[full_fit_no_pf$ind_min]
      new_support_coop_no_pf[ii, j] = full_fit_no_pf$nzero[cvm_min_ind]
    } else if (fit_mode == '1se'){
      cvm_min_ind = which.min(full_fit_no_pf$cvm)
      cvm_1se_ind = which(full_fit_no_pf$cvm < full_fit_no_pf$cvm[cvm_min_ind] + full_fit_no_pf$cvsd[cvm_min_ind])[1]
      new_cv_coop_no_pf[ii, j] = full_fit_no_pf$cvm[cvm_1se_ind]
      cvm_min_no_pf[j] = full_fit_no_pf$cvm[cvm_1se_ind]
      new_support_coop_no_pf[ii, j] = full_fit_no_pf$nzero[cvm_1se_ind]
    }
    
    new_train_coop_no_pf[ii,j] = calc_mse(yhat_coop_new_no_pf, mu_train)
    yhat_coop_new_test_no_pf = predict(full_fit_no_pf, list(x1=test_X1,x2=test_X2,x3=test_X3), s="lambda.min")
    ...
    full_fit = cv.multiview(list(x1=train_X1,x2=train_X2,x3=train_X3),train_y,
                            rho=alpha,foldid=foldid,penalty.factor=pfs)
    
    yhat_coop_new = predict(full_fit, list(x1=train_X1,x2=train_X2,x3=train_X3), s="lambda.min")
    new_train_coop[ii,j] = calc_mse(yhat_coop_new, mu_train)
    
    if (fit_mode == 'min'){
      cvm_min_ind = which.min(full_fit$cvm)
      new_cv_coop[ii, j] = min(full_fit$cvm) #full_fit_no_pf$cvm[full_fit_no_pf$ind_min]
      cvm_min[j] = min(full_fit$cvm) #full_fit_no_pf$cvm[full_fit_no_pf$ind_min]
      new_support_coop[ii, j] = full_fit$nzero[cvm_min_ind]
    } else if (fit_mode == '1se'){
      cvm_min_ind = which.min(full_fit$cvm)
      cvm_1se_ind = which(full_fit$cvm < full_fit$cvm[cvm_min_ind] + full_fit$cvsd[cvm_min_ind])[1]
      new_cv_coop[ii, j] = full_fit$cvm[cvm_1se_ind]
      cvm_min[j] = full_fit$cvm[cvm_1se_ind]
      new_support_coop[ii, j] = full_fit$nzero[cvm_1se_ind]
    }
    
    new_train_coop[ii,j] = calc_mse(yhat_coop_new, mu_train)
    yhat_coop_new_test = predict(full_fit, list(x1=test_X1,x2=test_X2,x3=test_X3), s="lambda.min")
