...
data = readRDS("./real_data/processed_data.Rdata")
#data = readRDS("./treatment_response/readytoanalyze.Rdata")
...
alphalist = c(0,0.3,0.6,0.9,1,3,5,9)
val_frac = 0.3
...
for (ii in 1:simN){
  ...
  X_ind = which(log(apply(train_X_raw,2,var)) > 0.6)
  Z_ind = which(log(apply(train_Z_raw,2,var)) > -0.3) 
  ...
  #X_yhat_lasso_val = predict(X_lasso_fit, val_X, s = "lambda.min")
  ...
  #Z_yhat_lasso_val = predict(Z_lasso_fit, val_Z, s = "lambda.min")
...
test_err = cbind(err_null, X_err_test_lasso, Z_err_test_lasso, err_fuse, err_test_lasso, 
                 coop_selected_by_cv_no_pf, coop_selected_by_cv, 
                 new_test_coop_no_pf, new_test_coop)
saveRDS(test_err, paste0("treatment_test_err.rds"))
...
support_df = cbind(X_lasso_support, Z_lasso_support,
                   ...
                   new_support_coop)
saveRDS(support_df, paste0("treatment_support.rds"))
...
pdf(file="./treatment_rna_cnv.pdf", width=15, height=5)
par(mfrow=c(1,3), las=2, cex.main=2.1, cex.axis = 1.8, mar=c(10.5,3.2,3.2,0.9))
boxplot(as.vector(test_err[,1:7])~sort(rep(1:(7+length(alphalist)*0), simN)), 
        names=c("Null", "Separate RNA", "Separate CNV", 
                "Late", "Early",  
                "Coop", "Adap Coop"), #alpha_axis), 
        ylim=c(1.0,3.5),
        #col = c(rep("white",4),"#ff8080", rep("#BCDEBC",length(alphalist))),
        xlab="", ylab="", main="Test MSE")

lasso_diff = cbind(test_err) - test_err[,5]
boxplot(as.vector(lasso_diff[,1:7])~sort(rep(1:(7+length(alphalist)*0), simN)), 
        names=c("Null", "Separate RNA", "Separate CNV", 
                "Late", "Early",  "Coop", "Adap Coop"), #alpha_axis), 
        ylim=c(-0.7,0.8),
        #col = c(rep("white",4),"#ff8080", rep("#BCDEBC",length(alphalist))),
        xlab="", ylab="", main="Test MSE Difference with Early Fusion")
abline(h=0, lty=2)

null_diff = cbind(test_err) - test_err[,1]
boxplot(as.vector(null_diff[,1:7])~sort(rep(1:(7+length(alphalist)*0), simN)), 
        names=c("Null", "Separate RNA", "Separate CNV", 
                "Late", "Early",  "Coop", "Adap Coop"), #alpha_axis), 
        ylim=c(-0.3,1.2),
        #col = c(rep("white",4),"#ff8080", rep("#BCDEBC",length(alphalist))),
        xlab="", ylab="", main="Test MSE Difference with Null")
abline(h=0, lty=2)

boxplot(as.vector(support_df[,1:6])~
          sort(rep(1:(6+length(alphalist)*0), simN)),
        #col = c(rep("white",4),"#ff8080", rep("#BCDEBC",length(alphalist))),
        names=c("Separate RNA","Separate CNV", "Late", "Early", "Coop", "Adap Coop"),#alpha_axis),
...
# pdf(file="path_treatment_rna_cnv.pdf", width=5, height=5)
...
#selected features
X_ind = which(log(apply(x_fil,2,var)) > 0.6)
Z_ind = which(log(apply(z_fil,2,var)) > -0.3)
